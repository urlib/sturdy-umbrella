...
{
template <typename T, T BigBase>
struct Utils
{
};
template <uint32_t BigBase>
struct Utils<uint32_t, BigBase>
{
    using W = uint32_t;
...
struct ModNumTypes
{
    ...
    public:
        ...
        ModNum pow(ModNum a, NumType k) const
        {
            ...
            {
                if (k & 1)
                    res = mul_lazy(res, base);
                if (k >>= 1)
                    base = mul_lazy(base, base);
                else
                    break;
            }
...
struct BigInt
{
    ...
    void normalize()
    {
        while (!empty() && IsZeroW(limbs.back()))
            limbs.pop_back();
    }
...
{
    if (pn < qn)
        return -_compare(q, qn, p, pn);
    for (int i = pn - 1; i >= qn; --i)
        if (!IsZeroW(p[i]))
            return 1;
